...
#ifdef DEBUG_LOCATION
#define INFO(...) if (Serial) Serial.printf("[INFO] %s:%d\t%s\r\n", __FILE__, __LINE__, stripString(formatString(__VA_ARGS__)).c_str())
#else
...
#ifdef DEBUG_LOCATION
#define ERROR(...) if (Serial) Serial.printf("[ERROR] %s:%d\t%s\r\n", __FILE__, __LINE__, stripString(formatString(__VA_ARGS__)).c_str())
#else
